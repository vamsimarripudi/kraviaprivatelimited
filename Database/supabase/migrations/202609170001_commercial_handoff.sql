...
-- This orchestrates preparation and approval only. It never executes contracts,
-- activates subscriptions, or issues invoices automatically.
...
create index if not exists office_commercial_handoffs_customer_idx on public.office_commercial_handoffs(customer_id,product_id,status);
create index if not exists office_commercial_handoffs_owner_idx on public.office_commercial_handoffs(owner_user_id,status);
...
create index if not exists office_commercial_handoff_events_handoff_idx on public.office_commercial_handoff_events(handoff_id,created_at desc);
...
select id,1,'LEGAL_REVIEW','Legal contract review','PERMISSION','legal.contract.review','legal.contract.review',1,false,'{}'::jsonb from public.office_workflow_templates where code='COMMERCIAL_CONTRACT_V1' and version=1
...
insert into public.office_workflow_template_steps(template_id,step_order,step_code,label,approver_selector,approver_value,required_permission,min_approvals,allow_self_approval,condition_json)
select id,2,'CONTRACT_EXECUTION','Authorised contract execution','PERMISSION','legal.contract.execute','legal.contract.execute',1,false,'{}'::jsonb from public.office_workflow_templates where code='COMMERCIAL_CONTRACT_V1' and version=1
on conflict(template_id,step_order) do update set step_code=excluded.step_code,label=excluded.label,approver_selector=excluded.approver_selector,approver_value=excluded.approver_value,required_permission=excluded.required_permission,min_approvals=excluded.min_approvals,allow_self_approval=excluded.allow_self_approval,condition_json=excluded.condition_json;
insert into public.office_workflow_template_steps(template_id,step_order,step_code,label,approver_selector,approver_value,required_permission,min_approvals,allow_self_approval,condition_json)
select id,1,'COMMERCIAL_REVIEW','Commercial readiness review','PERMISSION','sales.crm.write','sales.crm.write',1,false,'{}'::jsonb from public.office_workflow_templates where code='SUBSCRIPTION_ACTIVATION_V1' and version=1
on conflict(template_id,step_order) do update set step_code=excluded.step_code,label=excluded.label,approver_selector=excluded.approver_selector,approver_value=excluded.approver_value,required_permission=excluded.required_permission,min_approvals=excluded.min_approvals,allow_self_approval=excluded.allow_self_approval,condition_json=excluded.condition_json;
insert into public.office_workflow_template_steps(template_id,step_order,step_code,label,approver_selector,approver_value,required_permission,min_approvals,allow_self_approval,condition_json)
select id,2,'FINANCE_READINESS','Finance readiness review','PERMISSION','finance.read','finance.read',1,false,'{}'::jsonb from public.office_workflow_templates where code='SUBSCRIPTION_ACTIVATION_V1' and version=1
on conflict(template_id,step_order) do update set step_code=excluded.step_code,label=excluded.label,approver_selector=excluded.approver_selector,approver_value=excluded.approver_value,required_permission=excluded.required_permission,min_approvals=excluded.min_approvals,allow_self_approval=excluded.allow_self_approval,condition_json=excluded.condition_json;
insert into public.office_workflow_template_steps(template_id,step_order,step_code,label,approver_selector,approver_value,required_permission,min_approvals,allow_self_approval,condition_json)
select id,1,'FINANCE_PREPARATION','Finance billing preparation','PERMISSION','finance.invoice.create','finance.invoice.create',1,false,'{}'::jsonb from public.office_workflow_templates where code='INVOICE_PREPARATION_V1' and version=1
on conflict(template_id,step_order) do update set step_code=excluded.step_code,label=excluded.label,approver_selector=excluded.approver_selector,approver_value=excluded.approver_value,required_permission=excluded.required_permission,min_approvals=excluded.min_approvals,allow_self_approval=excluded.allow_self_approval,condition_json=excluded.condition_json;

create or replace function public.office_commercial_initialize_handoff(p_actor uuid,p_opportunity uuid)
returns uuid language plpgsql security definer set search_path='' as $$
declare opp public.office_crm_opportunities%rowtype; v_handoff_id uuid; owner_dept text;
begin
  ...
  insert into public.office_commercial_handoffs(opportunity_id,customer_id,product_id,owner_user_id,created_by,updated_by)
  values(opp.id,opp.customer_id,opp.product_id,opp.owner_user_id,p_actor,p_actor)
  on conflict(opportunity_id) do update set updated_by=excluded.updated_by,updated_at=now() returning id into v_handoff_id;
  if not exists(select 1 from public.office_commercial_handoff_events e where e.handoff_id=v_handoff_id and e.event_type='HANDOFF_INITIALIZED') then
    insert into public.office_commercial_handoff_events(handoff_id,actor_user_id,event_type,metadata) values(v_handoff_id,p_actor,'HANDOFF_INITIALIZED',jsonb_build_object('opportunity_id',p_opportunity));
  end if;
  return v_handoff_id;
end; $$;

create or replace function public.office_commercial_link_request(p_actor uuid,p_handoff uuid,p_kind text,p_request uuid)
returns text language plpgsql security definer set search_path='' as $$
declare h public.office_commercial_handoffs%rowtype; r public.office_requests%rowtype; expected_type text; normalized_kind text:=upper(trim(coalesce(p_kind,''))); owner_dept text;
begin
  ...
  expected_type:=case normalized_kind when 'CONTRACT' then 'COMMERCIAL_CONTRACT' when 'SUBSCRIPTION' then 'SUBSCRIPTION_ACTIVATION' when 'INVOICE' then 'INVOICE_PREPARATION' else null end;
  ...
  if normalized_kind='CONTRACT' then update public.office_commercial_handoffs set contract_request_id=p_request,updated_by=p_actor,updated_at=now() where id=p_handoff;
  elsif normalized_kind='SUBSCRIPTION' then update public.office_commercial_handoffs set subscription_request_id=p_request,updated_by=p_actor,updated_at=now() where id=p_handoff;
  else update public.office_commercial_handoffs set invoice_request_id=p_request,updated_by=p_actor,updated_at=now() where id=p_handoff; end if;
  insert into public.office_commercial_handoff_events(handoff_id,actor_user_id,event_type,metadata) values(p_handoff,p_actor,normalized_kind||'_REQUEST_LINKED',jsonb_build_object('request_id',p_request,'request_type',expected_type));
