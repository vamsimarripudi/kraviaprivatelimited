...
const PUBLIC_KEY_B64 =
  "MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEZsjNzn15B7PpWi49zIBscmXvHWxjuWDouCKauXKnP5AbpQu9h5HD0KLEkDfhoX+eKZsdRyoNneJ1y+bsK8Whuw==";
...
function adminClient() {
  ...
  if (modern) {
    try {
      ...
      // Hosted projects retain the legacy service-role fallback during key migration.
    }
  }
  ...
  try {
    ...
    if (action === "signed_download") {
      ...
      const expiresIn = Math.max(
        15,
        Math.min(Number.isFinite(requested) ? requested : 60, 300),
      );
    ...
    const code = error instanceof Error ? error.name : "StorageBrokerError";
    return Response.json(
      { detail: "Storage broker operation failed", code },
      { status: 502 },
    );
  }
});
