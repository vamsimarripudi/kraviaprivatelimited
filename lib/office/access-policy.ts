...
export const officeDepartments = ["EXECUTIVE","ADMINISTRATION","FINANCE","TAX","SECRETARIAL","LEGAL","PEOPLE","OPERATIONS","AUDIT","PRODUCT"] as const;
...
export const adminAssignableRoles: readonly OfficeRole[] = ["FINANCE", "CA", "CS", "LEGAL", "HR", "OPERATIONS", "AUDITOR", "PRODUCT_ADMIN"];
export type AccessDecision = { allowed: true } | { allowed: false; reason: string };

export function isOfficeDepartment(value: unknown): value is OfficeDepartment { return typeof value === "string" && (officeDepartments as readonly string[]).includes(value); }
export function activeRoleNames(rows: readonly { role: unknown; expires_at?: unknown }[], now = Date.now()): OfficeRole[] {
  const roles = rows.filter((row) => { if (typeof row.expires_at !== "string" || !row.expires_at) return true; const expiry = Date.parse(row.expires_at); return Number.isFinite(expiry) && expiry > now; }).map((row) => row.role).filter(isOfficeRole);
  return Array.from(new Set(roles));
}
export function hasRoleConflict(roles: readonly OfficeRole[]) { const set = new Set(roles); if (!set.has("AUDITOR")) return false; return ["OWNER","DIRECTOR","ADMIN","FINANCE"].some((role) => set.has(role as OfficeRole)); }
export function canAdministerAccess(roles: readonly OfficeRole[]) { return roles.includes("OWNER") || roles.includes("ADMIN"); }
...
function targetIsPrivileged(targetRoles: readonly OfficeRole[]) { return targetRoles.some((role) => ownerManagedRoles.includes(role)); }
...
export function canChangeRole(actorUserId:string,actorRoles:readonly OfficeRole[],targetUserId:string,targetRoles:readonly OfficeRole[],role:OfficeRole,action:"GRANT"|"REVOKE"):AccessDecision {
  const targetDecision=canManageTarget(actorUserId,actorRoles,targetUserId,targetRoles); if(!targetDecision.allowed)return targetDecision;
...
export function canChangeStatus(actorUserId:string,actorRoles:readonly OfficeRole[],targetUserId:string,targetRoles:readonly OfficeRole[],nextStatus:MutableOfficeIdentityStatus):AccessDecision {
  const targetDecision=canManageTarget(actorUserId,actorRoles,targetUserId,targetRoles); if(!targetDecision.allowed)return targetDecision;
  if(!(["ACTIVE","SUSPENDED","REVOKED"] as const).includes(nextStatus))return{allowed:false,reason:"Invalid identity status"}; return{allowed:true};
}
export function canReviewAccess(actorUserId:string,actorRoles:readonly OfficeRole[],targetUserId:string,targetRoles:readonly OfficeRole[]):AccessDecision { if(actorUserId===targetUserId)return{allowed:false,reason:"Access reviews require another authorised reviewer"}; return canManageTarget(actorUserId,actorRoles,targetUserId,targetRoles); }
export function suggestedDepartmentForRoles(roles:readonly OfficeRole[]):OfficeDepartment { if(roles.includes("ADMIN"))return"ADMINISTRATION";if(roles.includes("DIRECTOR"))return"EXECUTIVE";if(roles.includes("CA"))return"TAX";if(roles.includes("FINANCE"))return"FINANCE";if(roles.includes("CS"))return"SECRETARIAL";if(roles.includes("LEGAL"))return"LEGAL";if(roles.includes("HR"))return"PEOPLE";if(roles.includes("AUDITOR"))return"AUDIT";if(roles.includes("PRODUCT_ADMIN"))return"PRODUCT";return"OPERATIONS"; }
