...
const officeCore: readonly OfficeRole[] = ["OWNER", "DIRECTOR", "CS", "LEGAL", "HR", "OPERATIONS", "PRODUCT_ADMIN"];
...
export const officeSections = {
  dashboard: { title: "Office overview", eyebrow: "KRAVIA OFFICE", description: "Company operations, governed records and role-scoped work in one private workspace.", roles: officeCore, group: "Operate" },
  ...
  documents: { title: "Document vault", eyebrow: "PRIVATE EVIDENCE", description: "Versioned, hashed and access-controlled corporate records.", roles: officeCore, group: "Records" },
  ...
  people: { title: "People", eyebrow: "HR & ACCESS", description: "Internal people records and role-scoped administration.", roles: ["OWNER", "DIRECTOR", "HR"], group: "Office" },
  ...
  security: { title: "Security", eyebrow: "RESTRICTED ADMINISTRATION", description: "Identity, access and security-control administration for privileged operators.", roles: executive, group: "Assure" },
  ...
  settings: { title: "Office settings", eyebrow: "IDENTITY & CONTROL", description: "Workspace identity, MFA status, role assignments and controlled configuration.", roles: officeCore, group: "Assure" },
...
export const workspaceDefinitions = {
  office: {
    ...
    workspaceRoles: officeCore,
  },
  finance: {
    label: "KRAVIA FINANCE",
    basePath: "/finance",
    loginPath: "/finance/login",
    sections: financeSections,
    workspaceRoles: financeCore,
  },
} as const;
...
export function preferredWorkspace(roles: readonly OfficeRole[]): WorkspaceKind | null {
  if (roles.some((role) => ["OWNER", "DIRECTOR"].includes(role))) return "office";
  if (roles.some((role) => ["FINANCE", "CA", "AUDITOR"].includes(role))) return "finance";
  if (roles.some((role) => ["CS", "LEGAL", "HR", "OPERATIONS", "PRODUCT_ADMIN"].includes(role))) return "office";
  return null;
}
