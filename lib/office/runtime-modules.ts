...
const officeRuntimeModules: Partial<Record<OfficeSection, RuntimeModuleSpec>> = {
  dashboard: { path: "command-center", note: "Derived from canonical KRAVIA Office records." },
...
const financeRuntimeModules: Partial<Record<FinanceSection, RuntimeModuleSpec>> = {
  dashboard: { path: "command-center", note: "Canonical financial and exception summary." },
  ...
  payments: { path: "finance/payment-instructions", columns: [{ key: "id", label: "Instruction" }, { key: "amount", label: "Amount" }, { key: "provider", label: "Provider" }, { key: "status", label: "Status" }, { key: "created_at", label: "Created" }], note: "Maker-checker controlled payment instructions." },
  expenses: { path: "finance/expenses", columns: [{ key: "id", label: "Expense" }, { key: "description", label: "Description" }, { key: "amount", label: "Amount" }, { key: "due_date", label: "Due" }, { key: "status", label: "Status" }], note: "Expense funding never changes legal ownership." },
  ...
  ownership: { path: "ownership/summary", note: "Append-only ownership ledger; funding and expenses remain separate." },
