...
export function PageHero({ eyebrow, title, intro }: { eyebrow: string; title: string; intro: string }) { const reduce = useReducedMotion(); return <section className="page-hero shell"><p className="eyebrow">{eyebrow}</p><motion.h1 initial={reduce ? false : { opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .6, ease: [0.22, 1, 0.36, 1] }}>{title}</motion.h1><p className="lede">{intro}</p></section>; }
