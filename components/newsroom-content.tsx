...
export function NewsroomLanding({ articles }: { articles: readonly PublicContentRecord[] }) {
  ...
  return <section className="newsroom-shell shell">
    {featured ? <article className="newsroom-feature"><p className="eyebrow">FEATURED · {typeLabel(featured.type)}</p><h2>{featured.title}</h2><p>{featured.summary}</p><Link href={publicContentPath(featured)} className="text-link">Read the update <ArrowUpRight /></Link></article> : <div className="newsroom-empty"><FileText /><div><p className="eyebrow">OFFICIAL KRAVIA NEWSROOM</p><h2>Updates will appear here when they are ready.</h2><p>Kravia publishes company, product, engineering, research and press material only after it has been reviewed and approved.</p></div></div>}
    {latest.length > 0 && <div className="newsroom-list" aria-label="Latest updates">{latest.map((article) => <article key={article.id}><p className="eyebrow">{typeLabel(article.type)} · {dateLabel(article.publishedAt)}</p><h2><Link href={publicContentPath(article)}>{article.title}</Link></h2><p>{article.summary}</p><Link href={publicContentPath(article)} aria-label={`Read ${article.title}`}><ArrowUpRight /></Link></article>)}</div>}
  </section>;
}
...
export function HomepageUpdates({ articles }: { articles: readonly PublicContentRecord[] }) {
  ...
  return <section className="homepage-updates shell" aria-labelledby="homepage-updates-title"><div className="homepage-updates-heading"><div><p className="eyebrow">LATEST FROM KRAVIA</p><h2 id="homepage-updates-title">News with a clear source.</h2></div><Link className="text-link" href="/newsroom">View newsroom <ArrowUpRight aria-hidden="true" /></Link></div><div className="homepage-updates-grid">{updates.map((article) => <article key={article.id}><p className="eyebrow">{typeLabel(article.type)} · {dateLabel(article.publishedAt)}</p><h3><Link href={publicContentPath(article)}>{article.title}</Link></h3>{article.summary ? <p>{article.summary}</p> : null}<Link className="text-link" href={publicContentPath(article)}>Read update <ArrowUpRight aria-hidden="true" /></Link></article>)}</div></section>;
}

function publicationLabel(article: PublicContentRecord) {
  const reviewed = dateLabel(article.lastReviewedAt);
  const published = dateLabel(article.publishedAt);
  if (reviewed) return `Reviewed ${reviewed}`;
  return published ? `Published ${published}` : "Official public record";
}

/**
 * Every approved public record uses this reading surface—not only newsroom
 * stories. It gives Company, Product and Trust content a clear governed source
 * without making those pages look like a generic blog.
 */
export function PublicContentArticle({ article, allArticles }: { article: PublicContentRecord; allArticles: readonly PublicContentRecord[] }) {
  const related = relatedContent(article, allArticles);
  return <article className="article-shell shell">
    <header className="article-header"><p className="eyebrow">{typeLabel(article.type)}</p><h1>{article.title}</h1>{article.summary && <p className="article-summary">{article.summary}</p>}<div className="article-meta"><span>{publicationLabel(article)}</span><span>{article.authorName ? `By ${article.authorName}` : "KRAVIA PRIVATE LIMITED"}</span>{article.updatedAt && article.publishedAt && article.updatedAt !== article.publishedAt && <span>Updated {dateLabel(article.updatedAt)}</span>}<CopyLinkButton label="Copy link" /></div></header>
    <div className="article-reading">{paragraphs(article.body).map((paragraph, index) => <p key={index}>{paragraph}</p>)}</div>
    {related.length > 0 && <aside className="related-content"><p className="eyebrow">RELATED CONTENT</p><div>{related.map((item) => <Link key={item.id} href={publicContentPath(item)}><span>{typeLabel(item.type)}</span><strong>{item.title}</strong><ArrowUpRight /></Link>)}</div></aside>}
  </article>;
}

/** @deprecated Use PublicContentArticle for every governed public record. */
export const NewsArticle = PublicContentArticle;
