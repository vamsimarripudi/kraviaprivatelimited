...
function IdentityCard({ identity }: { identity: OfficeIdentity }) {
  ...
  if (roleCanAccessSection(officeSections.settings, identity.roles)) return <Link href="/office/settings" className="office-identity" aria-label="Open identity and security settings">{content}</Link>;
...
export function InternalWorkspaceScreen({ workspace, section, identity }: Props) {
  ...
  return <main className={`office office-v2 workspace-shell workspace-${workspace}`}>
    ...
    <section className="office-main"><header className="office-topbar"><div><p className="eyebrow">{item?.eyebrow ?? definition.label}</p><h1>{item?.title ?? definition.label}</h1></div><IdentityCard identity={identity} /></header>
      {permitted && item ? <><div className="office-notice"><ShieldCheck /><p>{item.description}</p></div>{workspace === "office" && section === "access" ? <AccessGovernancePanel identity={identity} /> : section === "dashboard" ? <WorkspaceDashboard workspace={workspace} section={section} identity={identity} /> : <WorkspaceModule workspace={workspace} section={section} item={item} />}</> : <section className="office-denied"><TriangleAlert /><div><p className="eyebrow">ACCESS RESTRICTED</p><h2>This module is not assigned to your role.</h2><p>KRAVIA Office permissions are enforced from the dedicated identity tenant. Access changes require an authorised role assignment and a newly issued or authoritatively rechecked session.</p><Link className="text-link" href={`${definition.basePath}/dashboard`}>Return to overview <ArrowRight /></Link></div></section>}
    </section>
  </main>;
}
...
function WorkspaceDashboard({ workspace, section, identity }: { workspace: WorkspaceKind; section: OfficeSection | FinanceSection; identity: OfficeIdentity }) {
  const sections = sectionEntries(workspace).filter(([slug, item]) => slug !== "dashboard" && roleCanAccessSection(item, identity.roles)).slice(0, 8);
  const basePath = workspaceDefinitions[workspace].basePath;
  const runtime = runtimeModuleSpec(workspace, section);
  const canReadCanonicalRuntime = identity.roles.some((role) => role !== "ADMIN");
  return <><section className="workspace-hero-panel"><div><p className="eyebrow">VERIFIED AAL2 SESSION</p><h2>{workspace === "finance" ? "Finance work without mixing ownership, tax and treasury." : identity.roles.length === 1 && identity.roles[0] === "ADMIN" ? "Access administration without automatic business-data privilege." : "One company workspace. Role-scoped by design."}</h2></div><ShieldCheck aria-hidden="true" /></section>
    {runtime && canReadCanonicalRuntime ? <WorkspaceRuntimePanel title={workspace === "finance" ? "Finance overview" : "Office overview"} spec={runtime} /> : null}
    <div className="office-dashboard-grid workspace-module-grid">{sections.map(([slug, item]) => <Link href={`${basePath}/${slug}`} key={slug} className="workspace-module-card"><p className="eyebrow">{item.group}</p><h2>{item.title}</h2><span>{item.description}</span><b>Open module <ArrowRight aria-hidden="true" /></b></Link>)}</div>
  </>;
}
...
function WorkspaceModule({ workspace, section, item }: { workspace: WorkspaceKind; section: OfficeSection | FinanceSection; item: WorkspaceSection }) {
  ...
  return <section className="office-record-panel workspace-record-panel" aria-label={`${item.title} workspace`}><div className="office-record-panel-head"><div><p className="eyebrow">CANONICAL COMPANY SYSTEM</p><strong>{workspace === "finance" ? "Finance & Tax" : "KRAVIA Office"}</strong></div><span className="office-status">ROLE SCOPED</span></div><div className="office-empty"><FileLock2 /><div><h2>Specialised workflow surface</h2><p>This module uses its controlled workflow rather than a generic table. No sample financial, legal, tax or corporate data is fabricated for presentation.</p></div></div></section>;
}
