...
export function EnquiryForm({ privacy = false, initialCategory, presentation }: EnquiryFormProps) {
  const [categoryOverride, setCategory] = useState<{ base: string | undefined; value: string } | null>(null);
  const category = categoryOverride && categoryOverride.base === initialCategory ? categoryOverride.value : initialCategory ?? (privacy ? "Privacy question" : categories[0]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (submitting.current) return; const form = new FormData(event.currentTarget); if (!validate(form)) return; submitting.current = true; requestId.current ??= crypto.randomUUID(); setStatus("submitting"); setFormError(undefined);
    try {
      const response = await fetch(privacy ? "/api/privacy-requests" : "/api/enquiries", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...Object.fromEntries(form), ...(!privacy ? { requestId: requestId.current } : {}) }) });
