...
class GstReturnWorking(Base):
    ...
    gsp_submission_ref = Column(String(160), nullable=True)
    gsp_response_hash = Column(String(64), nullable=True)
    gsp_status_json = Column(Text, nullable=False, default="{}")
    gsp_last_sync_at = Column(DateTime(timezone=True), nullable=True)
    gsp_verified_at = Column(DateTime(timezone=True), nullable=True)
    gsp_verified_status = Column(String(40), nullable=True)
    itc_review_json = Column(Text, nullable=True)
    itc_reviewed_by = Column(String(200), nullable=True)
    itc_reviewed_at = Column(DateTime(timezone=True), nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
    __table_args__ = (
