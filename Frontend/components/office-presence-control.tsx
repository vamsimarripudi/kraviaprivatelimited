...
export function OfficePresenceControl() {
  ...
  const heartbeat = useCallback(async () => {
    try {
      await fetch("/api/office-auth/heartbeat", { method: "POST", cache: "no-store", credentials: "same-origin" });
    } catch {
      /* Session heartbeat is security telemetry and never changes attendance state. */
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const initialHeartbeat = window.setTimeout(() => void heartbeat(), 5_000);
    const refresh = window.setInterval(() => void reload(), 30_000);
    const tick = window.setInterval(() => setClock(Date.now()), 1_000);
    const heartbeatTimer = window.setInterval(() => void heartbeat(), 120_000);
    const onVisible = () => { if (document.visibilityState === "visible") { void reload(); void heartbeat(); } };
  ...
  async function setAvailability(status: OfficeAvailabilityStatus) {
    ...
    try {
      announce(await json<WorkforceState>("/api/office-presence", {
        ...
        body: JSON.stringify({ status, until: status === "AVAILABLE" ? null : expiryIso(expiry), work_mode: state?.presence.work_mode ?? null, client_event_id: crypto.randomUUID() }),
      }));
    } catch (caught) {
  ...
  async function attendance(action: "CHECK_IN" | "BREAK_START" | "BREAK_END" | "CHECK_OUT") {
    ...
    try {
      ...
      announce(await json<WorkforceState>("/api/office-attendance", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action, work_mode: action === "CHECK_IN" ? workMode : undefined, time_zone: timeZone, client_event_id: crypto.randomUUID() }),
      }));
    } catch (caught) {
  ...
  return <details className={styles.control}>
    ...
    <div className={styles.panel}>
      ...
      {!state ? <div className={styles.loading}><LoaderCircle className="spin" /> Resolving live workforce state…</div> : <>
        ...
        <section><label>Availability</label><div className={styles.statusGrid}>{officeAvailabilityOptions.map((option) => <button key={option.value} type="button" disabled={busy} data-active={state.presence.effective_status === option.value} onClick={() => void setAvailability(option.value)}><span>{option.emoji}</span>{option.label}</button>)}</div></section>
        <section className={styles.expiryRow}><label htmlFor="presence-expiry">Auto-clear status</label><select id="presence-expiry" value={expiry} onChange={(event) => setExpiry(event.target.value as ExpirySelection)}><option value="NONE">No expiry</option><option value="1H">In 1 hour</option><option value="4H">In 4 hours</option><option value="EOD">End of day</option></select></section>
        <section className={styles.attendance}>
          ...
          {state.attendance.state === "OFF_CLOCK" ? <>
            <select aria-label="Work mode" value={workMode} onChange={(event) => setWorkMode(event.target.value as WorkModeSelection)}><option value="">Choose work mode</option>{officeWorkModeOptions.map((option) => <option key={option.value} value={option.value}>{option.emoji} {option.label}</option>)}</select>
