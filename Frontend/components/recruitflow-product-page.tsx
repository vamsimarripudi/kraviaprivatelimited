  ...
  return <main id="main-content" className={styles.page}>
    ...
    <section className={styles.hero}>
      <div className={`shell ${styles.heroGrid}`}>
        <div>
          <nav aria-label="Breadcrumb" className={styles.breadcrumb}><Link href="/products">Products</Link><span aria-current="page">RecruitFlow</span></nav>
          <p className="eyebrow">{recruitFlowProduct.hero.eyebrow}</p>
          <p className={styles.wordmark} aria-label="RecruitFlow">Recruit<span>Flow</span></p>
          ...
          <div className={styles.actions}>
            ...
            >
              Open RecruitFlow <ArrowUpRight aria-hidden="true" />
            </ProductTrackedLink>
            <ProductTrackedLink
        ...
        <aside className={styles.signal} aria-label="RecruitFlow product status">
    ...
    <section className={`shell ${styles.overview}`} aria-labelledby="recruitflow-overview-title">
      ...
      <div>
        <p>RecruitFlow is an India-focused recruitment SaaS operated by Kravia Private Limited. It connects recruiter work across company onboarding, roles, applications, candidate evidence, interviews, scorecards, outcomes, communications, analytics and subscription state.</p>
    ...
    <section className={styles.workflowSection} aria-labelledby="recruitflow-workflow-title">
      <div className="shell">
        <header className={styles.sectionHead}>
          ...
          <p>RecruitFlow separates internal evaluation, candidate-facing communication and platform state so each part of the hiring process stays legible.</p>
        </header>
    ...
    <section className={`shell ${styles.section}`} aria-labelledby="recruitflow-capabilities-title">
      <header className={styles.sectionHead}>
        ...
        <p>These capabilities are based on the implemented RecruitFlow repository and its current MVP scope rather than a generic recruitment-software feature list.</p>
      </header>
    ...
    <section id="market" className={styles.market} aria-labelledby="recruitflow-market-title">
      <div className="shell">
        <header className={styles.sectionHead}>
          ...
          <p>The estimates below come from named research publishers. They describe broader market categories, not RecruitFlow revenue, valuation, market share, customer count or guaranteed demand.</p>
        </header>
        ...
        <aside className={styles.marketMethod}>
          ...
          <div>
            ...
            <p>ATS, recruitment software and online recruitment technology are related but differently defined research categories. RecruitFlow publishes each source separately so methodology differences remain visible instead of producing a synthetic number that no publisher actually reported.</p>
    ...
    <section className={styles.boundary} aria-labelledby="recruitflow-boundary-title">
      <div className={`shell ${styles.boundaryGrid}`}>
        ...
        <div>
          ...
          <p>RecruitFlow can extract resume information and calculate deterministic matching evidence to help authorised recruiters review applicants. It does not independently hire, reject, disqualify or publish a consequential outcome.</p>
    ...
    <section className={`shell ${styles.section}`} aria-labelledby="recruitflow-standards-title">
      <header className={styles.sectionHead}>
        ...
        <p>RecruitFlow treats privacy, tenancy, application history and decision accountability as operating rules rather than marketing statements.</p>
      </header>
    ...
    <section className={styles.audience} aria-labelledby="recruitflow-audience-title">
      <div className="shell">
        <header className={styles.sectionHead}>
          <div><p className="eyebrow">07 / WHO RECRUITFLOW IS FOR</p><h2 id="recruitflow-audience-title">For teams that need hiring structure <em>without enterprise clutter.</em></h2></div>
          <p>RecruitFlow is positioned for Indian hiring teams that need a more accountable process across roles, candidates, interviews and outcomes.</p>
        </header>
    ...
    <section className={`shell ${styles.section} ${styles.faq}`} aria-labelledby="recruitflow-faq-title">
      ...
      <Accordion label="RecruitFlow frequently asked questions" entries={recruitFlowProduct.faqs.map(([title, content]) => ({ title, content: <p>{content}</p> }))} />
    </section>

    <section className={styles.final}>
      <div className={`shell ${styles.finalInner}`}>
        <div><p className="eyebrow">RECRUITFLOW / A KRAVIA PRODUCT</p><h2>Give hiring one accountable <em>system of record.</em></h2><p>Explore the RecruitFlow application or talk to Kravia about product fit.</p></div>
        <div className={styles.finalActions}>
          <ProductTrackedLink event="recruitflow_cta_clicked" product="recruitflow" href={recruitFlowProduct.website} external className="button button-light">Open RecruitFlow <ArrowUpRight aria-hidden="true" /></ProductTrackedLink>
