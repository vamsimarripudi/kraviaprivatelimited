...
type Session = { id: string; user_id: string; status: string; aal: string; mfa_verified: boolean; ip_address?: string | null; user_agent_hash?: string | null; started_at?: string | null; last_seen_at?: string | null; expires_at?: string | null; revoked_at?: string | null };
...
type AuthEvent = { id: string; user_id?: string | null; event_type: string; aal?: string | null; ip_address?: string | null; created_at: string };
...
type Payload = { scope: { type?: string | null; key?: string | null }; summary: { active_sessions: number; active_without_aal2: number; closed_or_expired_sessions: number; pending_devices: number; trusted_managed_devices: number; open_incidents: number }; people: Person[]; sessions: Session[]; devices: Device[]; auth_events: AuthEvent[]; incidents: Incident[]; access_audit: AccessEvent[]; disclaimer: string };
...
export function OfficeSecurityOverview() {
  ...
  const sessions = data.sessions.filter((item) => !needle || `${item.status} ${item.aal} ${people.get(item.user_id)?.display_name ?? ""} ${item.user_agent_hash ?? ""}`.toLowerCase().includes(needle));
