    ...
    {open ? <div className={styles.backdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
      <section className={styles.dialog} role="dialog" aria-modal="true" aria-label="KRAVIA Office command palette">
        <div className={styles.inputRow}>
          ...
          <input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search work, CRM, engineering or open a module…" aria-label="Search KRAVIA Office" />
          {loading ? <LoaderCircle className={styles.spin} aria-label="Searching" /> : null}
        ...
        <div className={styles.results}>
          ...
          {remote.length ? <div className={styles.group}><p>Canonical records</p>{remote.map((result) => <Link href={result.href} key={result.id} onClick={close}><span><b>{result.label}</b><small>{result.kind} · {result.meta}</small></span><ArrowUpRight aria-hidden="true" /></Link>)}</div> : null}
          {!local.length && !remote.length && !loading ? <div className={styles.empty}><Search /><b>No visible result</b><span>Search only returns records and modules within your current Office authority.</span></div> : null}
        </div>
