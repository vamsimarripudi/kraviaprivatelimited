...
describe("homepage product market context", () => {
  it("projects exactly the four approved product research records", () => {
    expect(portfolioMarketContexts.map((context) => context.productSlug)).toEqual(["vidyaluma", "nicerole", "yukta", "vorio"]);
  ...
  it("maps canonical corporate product routes without inventing market context", () => {
    ...
    expect(marketContextForProductHref("/products/nicerole")?.forecastValue).toBe("$122.9M");
    expect(marketContextForProductHref("/products/nicerole")?.source).toBe("IMARC Group");
