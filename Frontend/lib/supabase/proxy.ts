...
/** Optimistic website-admin route gate only. Server reads/actions and RLS enforce the actual authorisation boundary. */
export async function updateSession(request: NextRequest) {
  ...
  if (isLogin && user) {
    ...
    return NextResponse.redirect(new URL(destination?.startsWith("/admin") ? destination : "/admin", request.url));
  }
  return response;
}
