...
function taskSelect() {
  return "id,task_code,title,description,task_type,priority,status,assigned_user_id,created_by,department_code,source_type,source_key,source_request_id,due_at,started_at,blocked_at,blocked_reason,completed_at,cancelled_at,created_at,updated_at";
}
...
export async function getOfficeTaskInbox() {
  ...
  let query = admin.from("office_tasks").select(taskSelect()).order("created_at", { ascending: false }).limit(200);
  ...
  const eventsByTask = new Map<string, unknown[]>();
  for (const event of events ?? []) {
    const current = eventsByTask.get(event.task_id as string) ?? [];
    current.push(event);
    eventsByTask.set(event.task_id as string, current);
  }

  return {
    actor: { user_id: identity.userId, roles: identity.roles },
    assignable_people: people ?? [],
    tasks: (tasks ?? []).map((task) => ({ ...task, events: eventsByTask.get(task.id as string) ?? [] })),
  };
}
