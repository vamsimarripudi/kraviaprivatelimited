...
export async function resolveOfficePermission(
  ...
  if (deny) {
    return { allowed: false, permission: permissionCode, source: "OVERRIDE_DENY", scopeType: deny.scope_type as OfficeScopeType, scopeKey: deny.scope_key, reason: "Explicit permission deny" };
  }
  ...
  if (allowOverride) {
    return enforceDevice(admin, identity.userId, permissionCode, permissionResult.data, {
      ...
      scopeType: allowOverride.scope_type as OfficeScopeType,
      scopeKey: allowOverride.scope_key,
  ...
  for (const mapping of mappingResult.data ?? []) {
    ...
    return enforceDevice(admin, identity.userId, permissionCode, permissionResult.data, {
      ...
      permission: permissionCode,
      source: "PROFILE",
      profileCode: mapping.profile_code,
      scopeType: grantType,
      scopeKey: grantKey,
      reason: `Granted by ${mapping.profile_code}`,
    }, deviceId);
  }
...
async function enforceDevice(
  ...
  if (!enforced || (!permission.high_risk && !permission.requires_managed_device)) {
    return { ...decision, highRisk: permission.high_risk, requiresManagedDevice: permission.requires_managed_device };
  }
  if (!deviceId) {
